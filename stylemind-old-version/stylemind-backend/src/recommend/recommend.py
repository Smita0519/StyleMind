...
from src.recommend.filtering import SEASON_FALLBACK_ORDER
...
UNCERTAIN_SEASON_PENALTY_BASE = 0.05

def season_distance_penalty(item, weather_bucket):
    if item is None or not item.get("season_uncertain"):
        return 0.0
    season = item.get("season")
    if season in (weather_bucket, "all-season"):
        return 0.0
    order = SEASON_FALLBACK_ORDER.get(weather_bucket, [])
    if season in order:
        return UNCERTAIN_SEASON_PENALTY_BASE * (order.index(season) + 1)
    return UNCERTAIN_SEASON_PENALTY_BASE * (len(order) + 1)
...
def score_candidates(filtered, jackets, style_preference, knn_pool_size, weather_bucket):
    ...
    for pairing in pairings:
        ...
        for bottom, distance in pairing["matches"]:
            ...
            final_score -= season_distance_penalty(top, weather_bucket)      # ADD
            final_score -= season_distance_penalty(bottom, weather_bucket)   # ADD
            ...
            scored.append({
                ...
                "off_season": has_fallback_item(top, bottom, jacket) or any(
                    i is not None and i.get("season_uncertain") for i in (top, bottom, jacket)
                ),
            })
    ...
    for dress in get_dresses(filtered):
        ...
        dress_final_score -= season_distance_penalty(dress, weather_bucket)   # ADD
        scored.append({
            ...
            "off_season": has_fallback_item(dress, jacket) or any(
                i is not None and i.get("season_uncertain") for i in (dress, jacket)
            ),
        })
